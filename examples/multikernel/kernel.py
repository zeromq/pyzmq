...
class Kernel(object):
    ...
    def abort_queue(self):
        while True:
            try:
                ...
                client_id = self.reply_socket.recv(zmq.NOBLOCK)
            except zmq.ZMQError, e:
                if e.errno == zmq.EAGAIN:
                    break
            else:
    ...
    def execute_request(self, ident, client_id, parent):
        try:
        ...
        print>>sys.__stdout__, Message(reply_msg)
        self.reply_socket.send(ident, zmq.SNDMORE)
        self.reply_socket.send(client_id, zmq.SNDMORE)
        self.reply_socket.send_json(reply_msg)
        if reply_msg['content']['status'] == u'error':
            self.abort_queue()

    def complete_request(self, ident, client_id, parent):
        matches = {'matches' : self.complete(parent),
                   'status' : 'ok'}
        completion_msg = self.session.msg('complete_reply', matches, parent)
        self.reply_socket.send(ident, zmq.SNDMORE)
        self.reply_socket.send(client_id, zmq.SNDMORE)
        self.reply_socket.send_json(completion_msg)
    ...
    def start(self):
        while True:
            ...
            assert self.reply_socket.rcvmore(), "Unexpected missing message part."
            client_id = self.reply_socket.recv()
            assert self.reply_socket.rcvmore(), "Unexpected missing message part."
            msg = self.reply_socket.recv_json()
            omsg = Message(msg)
...
def main():
    ...
    rep_conn = connection % (port_base+10)
    pub_conn = connection % (port_base+11)
