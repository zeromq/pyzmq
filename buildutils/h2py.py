...
def process(fp, outfp, env = {}, log = sys.stderr):
    lineno = 0
    while 1:
        line = fp.readline()
        if not line: break
        ...
        if match:
            ...
            if importable.has_key(filename):
                ...
                if inclfp:
                    outfp.write(
                            '\n# Included from %s\n' % filename)
                    process(inclfp, outfp, env, log)
                else:
                    log.write('Warning - could not find file %s\n' %
                                     filename)
