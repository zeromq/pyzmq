...
bundled_version = (4,0,6)
libzmq = "zeromq-%i.%i.%i.tar.gz" % (bundled_version)
libzmq_url = "http://download.zeromq.org/" + libzmq
libzmq_checksum = "sha256:28a2a9c9b77014c39087a498942449df18bb9885cdb63334833525a1d19f2894"
...
def fetch_archive(savedir, url, fname, checksum, force=False):
    ...
    if digest != digest_ref:
        fatal("%s checksum %s doesn't match expected: %s" % (
            dest, digest, digest_ref))
    return dest
